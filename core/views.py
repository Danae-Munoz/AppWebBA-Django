...
def obtener_solicitudes_de_servicio(request):
    ...
    tipo_consulta = "Todos" if tipousu == "Administrador" else tipousu

    cursor.execute("EXEC SP_OBTENER_SOLICITUDES_DE_SERVICIO %s, %s", [tipo_consulta, rut])
...
def actualizar_solicitud_servicio(request):
    if request.method == "POST":
